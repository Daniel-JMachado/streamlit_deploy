...
if image_file is not None:  # Se uma imagem foi enviada
    try:
        # Conecta ao GitHub usando seu token
        g = Github(GITHUB_TOKEN)
        repo = g.get_repo(REPO_NAME)
        
        # Cria um nome único para o arquivo usando data/hora
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        # O arquivo será salvo em uma pasta 'images' no GitHub
        file_name = f"images/{timestamp}_{image_file.name}"
        
        # Converte a imagem para um formato que pode ser salvo no GitHub
        content = base64.b64encode(image_file.getvalue()).decode()
        ...
        # Mostra mensagem de sucesso com link para ver a imagem no GitHub
        st.success(f"Imagem salva com sucesso! [Ver no GitHub](https://github.com/{REPO_NAME}/blob/main/{file_name})")
        
    except Exception as e:
        st.error(f"Erro ao fazer upload: {str(e)}")  # Mostra erro se algo der errado
